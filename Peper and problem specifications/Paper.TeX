...
\end{tabular}
\end{table}
Looking at the results, one can immediately see how the trends are completely reversed. This is because it is not mandatory that the parameters that work best in centralized also work best in FedAvg. Given these results, we decided to run each of our experiments with a momentum equal to 0.5, a learning rate equal to 0.1, and SGD as the optimizer.
...
The first test performed is the comparison between FedMix and FedAvg in \textbf{IID} settings. 
\begin{table}[H]
...
\end{tabular}
\end{table}
In this case, our method does not show any improvement it performs worse. However, a look at the graph Fig.\ref{fig:appendix1} (in the appendix) shows that our method performs better at low epochs.
...
In this case, it can be seen that our proposed method is far better. The delta between the methods is very high and at the $50^{th}$ epoch is 6\%. Looking also at the graph Fig.\ref{fig:appendix2} (in the appendix), one can see how the FedMix performs better in most epochs. 
...
\end{tabular}
\end{table}
As before, our combination of normalization does not benefit from this type of option. In addition, looking at the graphs in the appendix (Fig.\ref{fig:appendix4} and Fig.\ref{fig:appendix5}) we can see that batch normalization is always the best.

Looking at when the distribution is \textbf{Non-IID unbalanced}, we can see how the trend does not seem to be reversing (The first table always refers to a local batch size equal to 128 and the second to 16).
\begin{table}[H]
\begin{tabular}{|p{0.12\columnwidth}|p{0.12\columnwidth}|p{0.12\columnwidth}|p{0.12\columnwidth}|p{0.12\columnwidth}|p{0.12\columnwidth}|}
\hline
& {\textbf{\begin{tabular}[c]{@{}c@{}c@{}}FedGKT \\ GN\end{tabular}}} 
...
However, taking a look at the graphs in the appendix (Fig.\ref{fig:appendix6} and Fig.\ref{fig:appendix7}), one can see that there is actually no clear distinction as to which method works best. Perhaps continuing communications around the experiments would have led to more stable results. Unfortunately, due to the several hours to carry out the runs, this was impossible for us to verify.
...
\end{tabular}
\end{table}
We then decided that we wanted to try to change the normalization on the client according to the 'chosen' batch size. This is because the paper \cite{b2} shows that group normalization has higher accuracies at low batch sizes, but if the latter is raised, the batch works better. To do this we have decided to use the ResNet-50 mix on the client, where the value of $\alpha_b$ will be equal to 0, while that of $\alpha_g$ will be equal to 1 if the batch size is less than 8 and vice versa if greater than 16. (In the discussion section, it will be explained why a ResNet-50 mix and not simply the ResNet-50 in batch or group normalization based on the batch size value).
The next problem that arose was how to 'aggregate' these two normalizations on the server. 
...
Subsequently, as usual, the data were also tested when distributed in \textbf{Non-IID balanced} and \textbf{Non-IID unbalanced}. The next two tables show the data in the respective methods.
...
\begin{table}[htb]
\caption{Comparison between methods related to data distribution}
\begin{tabular}{|p{0.15\columnwidth}|p{0.12\columnwidth}|p{0.12\columnwidth}|p{0.12\columnwidth}|p{0.12\columnwidth}|p{0.12\columnwidth}|}
\hline
& {\textbf{\begin{tabular}[c]{@{}c@{}c@{}}FedMix\\ $\alpha_b$=0.1 \\ $\alpha_g$=0.9\end{tabular}}} 
& {\textbf{\begin{tabular}[c]{@{}c@{}c@{}}FedAvg \end{tabular}}}
& {\textbf{\begin{tabular}[c]{@{}c@{}c@{}}FedGKT \end{tabular}}}
& {\textbf{\begin{tabular}[c]{@{}c@{}c@{}}FedADP \end{tabular}}} 
& {\textbf{\begin{tabular}[c]{@{}c@{}c@{}}Best \\Method\end{tabular}}} \\ \hline



{\textbf{IID}} & \rule[-4mm]{0mm}{1cm} {53.67} & \centerline{61.51} & \centerline{\textbf{84.47}} & \centerline{38.45} & \centerline{\textbf{FedGKT}}\\\hline

{\textbf{\begin{tabular}[c]{@{}c@{}}Non-IID \\ Balanced \end{tabular}}} & \rule[-4mm]{0mm}{1cm} {\textbf{25.99}} & \centerline{19.52} &\centerline{{\begin{tabular}[c]{@{}c@{}c@{}}Not \\Tested\end{tabular}}}& \centerline{18.18} & \centerline{\textbf{FedMixAvg}}\\\hline

{\textbf{\begin{tabular}[c]{@{}c@{}c@{}}Non-IID \\ Unbalanced\end{tabular}}} & \rule[-4mm]{0mm}{1cm} {\textbf{27.22}} & \centerline{22.99} & \centerline{26.46} & \centerline{22.35} & \centerline{\textbf{FedMixAvg}}\\\hline
